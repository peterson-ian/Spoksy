...
{
    public class DeleteMessageCommandTests : IntegrationTestBase
    {
        ...
        private readonly IDeleteMessageCommandHandler _handler;
        ...
        public DeleteMessageCommandTests() : base()
        {
        ...
        public async Task Handle_WithValidCommand_ShouldEditMessage()
        {
            ...
            Assert.Equal("Message deleted successfully.", result.Value);

            var message = await _messageRepository.GetByIdAsync(_existingMessage.Id);
            Assert.Null(message); 
        }
